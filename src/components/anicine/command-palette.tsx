...
export function CommandPalette() {
  ...
  const commands: Cmd[] = useMemo(() => {
    const nav: Cmd[] = [
      { id: "n-search", kind: "nav", label: "Open Search Catalogs", shortcut: "S", icon: <Search className="w-4 h-4 text-[#3B82F6]" />, run: () => { setMainTool("search"); goto("hero"); } },
      { id: "n-downloader", kind: "nav", label: "Open Link Downloader", shortcut: "D", icon: <Link2 className="w-4 h-4 text-[#06B6D4]" />, run: () => { setMainTool("downloader"); goto("hero"); } },
      { id: "n-catalog", kind: "nav", label: "Jump to Media Catalog", shortcut: "C", icon: <Layers className="w-4 h-4 text-[#10B981]" />, run: () => goto("catalog") },
      ...
      { id: "n-faq", kind: "nav", label: "Jump to FAQ", shortcut: "F", icon: <Film className="w-4 h-4 text-[#94A3B8]" />, run: () => goto("faq-section") },
      { id: "n-queue", kind: "nav", label: "Open Downloads Monitor", shortcut: "Q", icon: <Download className="w-4 h-4 text-[#06B6D4]" />, run: () => setDrawer(true) },
    ];
    ...
    const titles: Cmd[] = TRENDING_MEDIA.slice(0, 12).map((t) => ({
      ...
      icon: <Film className="w-4 h-4 text-[#94A3B8]" />,
      run: () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      <div className="absolute inset-0 bg-[#0B0F17]/90 backdrop-blur-sm" onClick={close} />
      <div className="relative bg-[#1B2433] border border-[#2D3D54] rounded-xl w-full max-w-lg overflow-hidden shadow-2xl animate-scale-in flex flex-col">
        {/* Input */}
        <div className="relative flex items-center border-b border-[#1E2A3C] p-3">
          <Search className="w-4 h-4 text-[#3B82F6] ml-1 mr-2.5 shrink-0" />
          <input
            ref={inputRef}
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Type a command or search catalogs..."
            className="w-full bg-transparent border-0 text-white placeholder:text-[#64748B] focus:outline-none text-sm font-semibold"
          />
          <kbd className="px-2 py-0.5 rounded bg-[#131A26] border border-[#1E2A3C] text-[10px] font-mono text-[#64748B]">ESC</kbd>
        </div>
          ...
          {results.length === 0 ? (
            ...
            groups.map((g) => (
              <div key={g.kind} className="mb-2">
                <div className="px-2 py-1 text-[10px] font-mono uppercase tracking-wider text-[#64748B]">{g.label}</div>
                {g.items.map((c) => {
                  flatIndex++;
                  const idx = flatIndex;
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active === idx ? "bg-[#243044]" : "hover:bg-[#243044]/60"
                      )}
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {c.hint && <div className="text-[10px] font-mono text-[#64748B] truncate">{c.hint}</div>}
                        </div>
                      </div>
                      {c.shortcut && (
                        <span className="text-[10px] font-mono text-[#64748B] bg-[#131A26] border border-[#1E2A3C] px-1.5 py-0.5 rounded shrink-0">
                          {c.shortcut}
                        </span>
                      )}
                    </button>
        ...
        {/* Footer */}
        <div className="p-2.5 bg-[#131A26] border-t border-[#1E2A3C] flex items-center justify-between text-[11px] text-[#64748B]">
