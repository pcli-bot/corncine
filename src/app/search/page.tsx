  ...
  return (
    <main className="min-h-screen bg-[#0B0E15] text-[#F8FAFC] px-4 py-10 max-w-5xl mx-auto">
      <nav className="text-sm text-[#B3B7C1] mb-6">
        <Link href="/" className="hover:text-[#F8FAFC]">CornCine</Link> <span className="mx-2">/</span> <span className="text-[#E7EAEF]">Search</span>
      </nav>
      <h1 className="text-3xl font-extrabold mb-2">{query ? `Search: ${query}` : "Search CornCine"}</h1>
      <p className="text-[#B3B7C1] mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-10">
        {top.map((item) => (
          <Link key={item.slug} href={`/search?q=${encodeURIComponent(item.title)}`} className="p-3 rounded-lg bg-white/5 border border-white/10 hover:border-[#EC69AE] transition flex flex-col">
            ...
            <span className="text-xs text-[#B3B7C1] mt-1 line-clamp-2">{item.desc}</span>
          </Link>
        ))}
      </div>

      <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
        <p className="text-sm text-[#B3B7C1] mb-2">Want universal search across all 27+ adult indexers?</p>
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0B0E15] flex items-center justify-center text-[#B3B7C1]">Loading search...</div>}>
      <SearchContent />
    </Suspense>
  );
}
